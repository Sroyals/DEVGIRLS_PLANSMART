...
class Service(db.Model):
    ...
    def as_dict(self):
        # Split descriptions and prices by commas
        subcategories = self.description.split(',')
        prices = self.price.split(',')

        # Form a list of dictionaries with subcategory and price pairs
        subcategory_price_pairs = [
            {"subcategory": subcat.strip(), "price": price.strip()} 
            for subcat, price in zip(subcategories, prices)
        ]
        ...
        return {
            ...
            "subcategory_price_pairs": subcategory_price_pairs
        }
...
class Order(db.Model):
    ...
    items_data = db.Column(db.String(255), nullable=False)
    price_data = db.Column(db.String(255), nullable=False)
    ...
    def as_dict(self):
        # Split items, prices, and quantities by commas
        items = self.items_data.split(',')
        prices = self.price_data.split(',')
        # Check if quantity is a string, convert to string if not, then split
        if not isinstance(self.quantity, str):
            quantity_str = str(self.quantity)  # Convert to string
        else:
            quantity_str = self.quantity  # Already a string

        quantities = quantity_str.split(',')


        # Form a list of dictionaries with item, price, and quantity pairs
        item_price_quantity_pairs = [
            {"item": item.strip(), "price": price.strip(), "quantity": quantity.strip()}
        ...
        return {
            ...
            "item_price_quantity_pairs": item_price_quantity_pairs
        }
